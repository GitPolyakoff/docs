...
If your API key is exposed publicly (e.g. committed to a repository), rotate it immediately — see [Leaked API Keys & Secrets](/guides/developers/leaked-secrets/).
...
Public user objects include a `steamId` field (SteamID64 string, or `null` if unlinked). You can also resolve a profile with `GET /users/{steamId}` or the dedicated `GET /users/steam/{steamId}` route.
...
For Node.js integrations, you can use the official client instead of hand-written `fetch` calls. See [TypeScript SDK](/guides/developers/sdk/).
...
- [Leaked API Keys & Secrets](/guides/developers/leaked-secrets/) (remediation if a key is exposed)
